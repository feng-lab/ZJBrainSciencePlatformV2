...
def upgrade() -> None:
    ...
    op.add_column("notification", sa.Column("type", sa.String(length=20), nullable=False))
    op.drop_index("ix_message_create_at", table_name="notification")
    op.drop_index("ix_message_receiver", table_name="notification")
    op.create_index(op.f("ix_notification_create_at"), "notification", ["create_at"], unique=False)
...
def downgrade() -> None:
    # ### commands auto generated by Alembic - please adjust! ###
    op.add_column("notification", sa.Column("msg_type", mysql.VARCHAR(length=20), nullable=False))
