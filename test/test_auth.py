...
@pytest.mark.parametrize("username,password", [("not exists user", "some password"), (ROOT_USERNAME, "wrong password")])
def test_login_wrong_username_or_password(username: str, password: str):
    login_form = {"grant_type": "password", "username": "not exists user", "password": ROOT_PASSWORD}
