...
@router.post("/api/createExperiment", description="创建实验", response_model=CreateExperimentResponse)
async def create_experiment(request: CreateExperimentRequest, _user: User = Depends(get_current_user_as_researcher())):
...
@router.get("/api/getExperimentInfo", description="获取实验详情", response_model=GetExperimentInfoResponse)
