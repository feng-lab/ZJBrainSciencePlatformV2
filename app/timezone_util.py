...
def convert_timezone_before_save(model: Model):
    return modify_model_field_by_type(model, datetime, lambda dt: convert_timezone(dt, CURRENT_TIMEZONE, timezone.utc))


def convert_timezone_after_get_db(model: Model):
    return modify_model_field_by_type(model, datetime, lambda dt: convert_timezone(dt, timezone.utc, CURRENT_TIMEZONE))
